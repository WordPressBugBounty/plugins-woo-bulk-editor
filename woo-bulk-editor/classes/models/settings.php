...
final class WOOBE_SETTINGS {
	...
	public function __construct() {
		...
		if ( ! isset( $role[0] ) && ( current_user_can( 'administrator' ) || array_intersect( apply_filters( 'woobe_permit_special_roles', array( 'administrator' ) ), $role ) ) ) {
	...
	public function get_fields( $use_roles = true ) {
					...
					foreach ( $options['fields'] as $key => $v ) {
						...
						if ( isset( $v['shop_manager_visibility'] ) ) {// because for shop manager its doesn exists
							if ( ! in_array( $key, array( '__checker', 'ID' ) ) ) {// this fields must be always visible!!
								if ( in_array( $this->current_user_role, apply_filters( 'woobe_permit_special_roles', array( 'administrator' ) ) ) ) {
		...
		if ( ! in_array( $this->current_user_role, apply_filters( 'woobe_permit_special_roles', array( 'administrator' ) ) ) and $use_roles ) {
