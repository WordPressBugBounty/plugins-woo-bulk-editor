...
// +++
// libs
require WOOBE_PATH . 'lib/storage.php';

// data
require_once WOOBE_DATA_PATH . 'fields.php';
require_once WOOBE_DATA_PATH . 'settings.php';

// classes
require WOOBE_PATH . 'classes/helper.php';
require WOOBE_PATH . 'classes/models/profiles.php';
require WOOBE_PATH . 'classes/models/settings.php';
require WOOBE_PATH . 'classes/models/products.php';
...
final class WOOBE {
	...
	public function init() {
		...
		if ( isset( $_GET['page'] ) and $_GET['page'] == 'woobe' ) {
			...
			add_action(
				...
				function () {
					$user_id = get_current_user_id();
					if ( isset( $_GET['woobe-notice-dismissed'] ) && isset( $_GET['notice_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['notice_nonce'] ) ), 'woobe_notice_nonce' ) ) {
		...
		add_action(
			...
			function () {
				add_submenu_page(
					...
					'BEAR ' . esc_html__( 'Bulk Editor', 'woo-bulk-editor' ),
					'BEAR ' . esc_html__( 'Bulk Editor', 'woo-bulk-editor' ),
					'manage_woocommerce',
					'woobe',
					function () {
	...
	public function admin_enqueue_scripts() {
		if ( isset( $_GET['page'] ) and $_GET['page'] == 'woobe' ) {
			...
			<script>
				...
				var woobe_assets_link = "<?php echo esc_attr( WOOBE_ASSETS_LINK ); ?>";
			...
			?>
				...
				var woobe_lang = '<?php echo esc_html( apply_filters( 'woobe_current_language', '' ) ); ?>';//for translating compatibilities
			</script>
			...
			function woobe_enqueue_theme_css() {
				...
				if ( file_exists( $child_css_path ) ) {
					wp_enqueue_style( 'woobe-theme-css', $child_css_url, array(), filemtime( $child_css_path ) );
				} elseif ( file_exists( $parent_css_path ) ) {
					wp_enqueue_style( 'woobe-theme-css', $parent_css_url, array(), filemtime( $parent_css_path ) );
				} else {
					// do nothing
				}
			}
			...
			wp_enqueue_style( 'woobe_datatables_redesign', WOOBE_ASSETS_LINK . 'css/tables-redesign.css', array(), WOOBE_VERSION );
			wp_enqueue_script( 'woobe_datatables_net', WOOBE_ASSETS_LINK . 'js/jquery.dataTables.js', array( 'jquery' ), WOOBE_VERSION, false );

			wp_enqueue_script( 'woobe_data_tables', WOOBE_ASSETS_LINK . 'js/data-tables.js', array( 'woobe_datatables_net' ), WOOBE_VERSION, false );

			wp_enqueue_style( 'woobe_data_tables_fc', WOOBE_ASSETS_LINK . 'css/fixedColumns.dataTables.min.css', array(), WOOBE_VERSION );
			wp_enqueue_script( 'woobe_data_tables_fc', WOOBE_ASSETS_LINK . 'js/dataTables.fixedColumns.min.js', array(), WOOBE_VERSION, true );

			wp_enqueue_script( 'woobe_jquery_growl', WOOBE_ASSETS_LINK . 'js/jquery.growl.js', array( 'jquery' ), WOOBE_VERSION, false );

			wp_enqueue_style( 'woobe_switchery', WOOBE_ASSETS_LINK . 'js/switchery/switchery.min.css', array(), WOOBE_VERSION );
			wp_enqueue_script( 'woobe_switchery', WOOBE_ASSETS_LINK . 'js/switchery/switchery.min.js', array( 'jquery' ), WOOBE_VERSION, false );

			wp_enqueue_style( 'woobe_chosen', WOOBE_ASSETS_LINK . 'js/chosen/chosen.min.css', array(), WOOBE_VERSION );
			wp_enqueue_script( 'woobe_chosen', WOOBE_ASSETS_LINK . 'js/chosen/chosen.jquery.min.js', array( 'jquery' ), WOOBE_VERSION, false );
			...
			wp_enqueue_script( 'woobe_autocomplete', WOOBE_ASSETS_LINK . 'js/easy-autocomplete/jquery.easy-autocomplete.js', array( 'jquery' ), WOOBE_VERSION, false );
			...
			wp_enqueue_script( 'woobe_datetimepicker_moment', WOOBE_ASSETS_LINK . 'js/datepicker/moment-with-locales.min.js', array( 'jquery' ), WOOBE_VERSION, false );
			wp_enqueue_script( 'woobe_datetimepicker', WOOBE_ASSETS_LINK . 'js/bootstrap-material-datetimepicker/bootstrap-material-datetimepicker.js', array( 'jquery' ), WOOBE_VERSION, false );

			wp_enqueue_script( 'woobe_placeholder_label', WOOBE_ASSETS_LINK . 'js/jquery.placeholder.label.min.js', array( 'jquery' ), WOOBE_VERSION, false );
			wp_enqueue_script( 'woobe_tooltip', WOOBE_ASSETS_LINK . 'js/tooltip.js', array( 'jquery' ), WOOBE_VERSION, false );

			wp_enqueue_script( 'woobe_tabs', WOOBE_ASSETS_LINK . 'js/tabs.js', array(), WOOBE_VERSION, false );
			wp_enqueue_script( 'woobe_scrollbar', WOOBE_ASSETS_LINK . 'js/jquery.scrollbar.min.js', array(), WOOBE_VERSION, false );
			// wp_enqueue_script('woobe_scrollbar', WOOBE_ASSETS_LINK . 'js/perfect-scrollbar.js', array(), WOOBE_VERSION, false);
			// ***
			wp_enqueue_script( 'woobe', WOOBE_ASSETS_LINK . 'js/woobe.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable', 'woobe_tabs' ), WOOBE_VERSION, false );
	...
	public function print_plugin_options() {
		...
		WOOBE_HELPER::render_html_e( WOOBE_PATH . 'views/woobe.php', apply_filters( 'woobe_print_plugin_options', $args ) );
	}
	...
	public function get_downloads() {
		...
		WOOBE_HELPER::render_html_e(
			WOOBE_PATH . 'views/parts/product-downloads.php',
			array(
	...
	public function woobe_get_gallery() {
		...
		WOOBE_HELPER::render_html_e(
			WOOBE_PATH . 'views/parts/product-gallery.php',
			array(
	...
	public function woobe_get_upsells() {
		...
		WOOBE_HELPER::render_html_e(
			WOOBE_PATH . 'views/parts/product-upsells.php',
			array(
	...
	public function woobe_get_cross_sells() {
		...
		WOOBE_HELPER::render_html_e(
			WOOBE_PATH . 'views/parts/product-cross-sells.php',
			array(
	...
	public function woobe_get_grouped() {
		...
		WOOBE_HELPER::render_html_e(
			WOOBE_PATH . 'views/parts/product-grouped.php',
			array(
		...
		switch ( $this->settings->active_fields[ $field_key ]['edit_view'] ) {
			...
			case 'multi_select':
				$res = WOOBE_HELPER::render_html(
					WOOBE_PATH . 'views/elements/multi_select.php',
					array(
						'field_key'     => $field_key,
						'product_id'    => $product_id,
						'val'           => $val,
			...
			case 'attr_visibility':
				...
				$res        = WOOBE_HELPER::render_html(
					WOOBE_PATH . 'views/elements/attribute_visibility.php',
					array(
						'field_key'  => $field_key,
			...
			case 'switcher':
				...
				$res = WOOBE_HELPER::draw_advanced_switcher(
					( $val == $values[0] ? true : false ),
					$product_id . '_' . $field_key,
					$field_key,
					array(
						'true'  => $labels[0],
						'false' => $labels[1],
			...
			case 'checkbox':
				...
				$res = WOOBE_HELPER::draw_checkbox(
					array(
	...
	private function is_should_init() {
		...
		if ( defined( 'DOING_AJAX' ) ) {
			...
			// Allow bear-ai add-on actions to trigger full initialization
			if ( isset( $_REQUEST['action'] ) && strpos( $_REQUEST['action'], 'bear_ai_' ) !== false ) {
				$init = true;
			}
		}
		...
		add_action(
			...
			<div class="notice notice-info" id="pn_<?php echo esc_attr( $slug ); ?>_ask_favour" style="position: relative;">
				...
				<div id="pn_<?php echo esc_attr( $slug ); ?>_review_suggestion">
					...
					<p><a href="javascript: pn_<?php echo esc_attr( $slug ); ?>_set_review(1); void(0);"><?php esc_html_e( 'Yes, I love it', 'woo-bulk-editor' ); ?></a> 🙂 | <a href="javascript: pn_<?php echo esc_attr( $slug ); ?>_set_review(0); void(0);"><?php esc_html_e( 'Not really...', 'woo-bulk-editor' ); ?></a></p>
				</div>
				...
				?>
					...
					<p>
						...
						<a href="javascript: pn_<?php echo esc_attr( $slug ); ?>_dismiss_review(1); void(0);" style="display: inline-block; margin-right: 10px;"><?php esc_html_e( 'Nope, maybe later', 'woo-bulk-editor' ); ?></a>
						<a href="javascript: pn_<?php echo esc_attr( $slug ); ?>_dismiss_review(2); void(0);"><?php esc_html_e( 'I already did', 'woo-bulk-editor' ); ?></a>
					</p>
				</div>
