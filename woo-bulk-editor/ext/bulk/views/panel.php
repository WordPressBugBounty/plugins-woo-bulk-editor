...
<div class="notice notice-warning">
	<p>
		<?php
		// translators: %s: number of products or scope description.
		printf( esc_html__( 'Bulk editing will be applied to: %s', 'woo-bulk-editor' ), '<span class="woobe_action_will_be_applied_to">' . esc_html__( 'all the products on the site', 'woo-bulk-editor' ) . '</span>' )
		?>
	</p>
</div>
...
function woobe_bulk_draw_text( $bulk_fields ) {
	...
	<?php foreach ( $bulk_fields as $field_key => $field ) : ?>
		...
		<?php if ( $fields[ $field_key ]['edit_view'] == 'meta_popup_editor' ) : ?>
			<div class="col-lg-4">
				<div class='filter-unit-wrap <?php echo esc_attr( $fields[ $field_key ]['css_classes'] ); ?>
				<?php
				if ( ! $fields[ $field_key ]['direct'] ) :
					?>
					woobe-direct-field<?php endif; ?>'>
					<div class="col-lg-1">
		...
		<?php else : ?>
			<div class="col-lg-4">
				<div class='filter-unit-wrap <?php echo esc_attr( $field['css_classes'] ); ?>
				<?php
				if ( ! $fields[ $field_key ]['direct'] ) :
					?>
					woobe-direct-field<?php endif; ?>'>
	...
	<div class="col-lg-1">
		...
		WOOBE_HELPER::draw_tooltip(
			sprintf(
				...
				esc_html__( 'Select how to round float values fractions in the numeric fields. Works for prices and numeric meta fields. Read more %s.', 'woo-bulk-editor' ),
...
function woobe_bulk_draw_taxonomies() {
	...
	if ( ! empty( $taxonomy_objects ) ) {
		foreach ( $taxonomy_objects as $t ) {
			...
			?>
			<div class="col-lg-4">
				<div class='filter-unit-wrap not-for-variations 
				<?php
				if ( ! $WOOBE->settings->get_fields()[ $t->name ]['direct'] ) :
					?>
					woobe-direct-field<?php endif; ?>' style="overflow: visible;">
...
function woobe_bulk_draw_add1( $fields ) {
	...
	<?php if ( $show_field ) : ?>
		<div class="col-lg-3">
			<div class='filter-unit-wrap <?php echo esc_attr( $fields[ $current_field_key ]['css_classes'] ); ?>
			<?php
			if ( ! $fields[ $current_field_key ]['direct'] ) :
				?>
				woobe-direct-field<?php endif; ?>'>
				<div class="col-lg-1">
					<div class="height4"></div>
			...
			<?php if ( $show_field ) : ?>
				<div class="col-lg-3">
					...
					if ( ! $fields[ $current_field_key ]['direct'] ) :
						...
						<div class="col-lg-1">
							<?php if ( $fields[ $current_field_key ]['direct'] ) : ?>
								<input type="checkbox" title='<?php esc_html_e( 'select it to use', 'woo-bulk-editor' ); ?>' name="woobe_bulk[is][<?php echo esc_attr( $current_field_key ); ?>]" class="bulk_checker" data-field-key="<?php echo esc_html( $current_field_key ); ?>" data-title="<?php echo esc_html( $fields[ $current_field_key ]['title'] ); ?>" value="1" /><br />
							<?php endif; ?>
						</div>
						<div class="col-lg-11">
							<div class="content-wrap">
	...
	<?php foreach ( $filter_keys2 as $current_field_key ) : ?>
		...
		<div class="col-lg-3">
			<div class='filter-unit-wrap <?php echo esc_attr( $fields[ $current_field_key ]['css_classes'] ); ?>
			<?php
			if ( ! $WOOBE->settings->get_fields()[ $current_field_key ]['direct'] ) :
				?>
				woobe-direct-field<?php endif; ?>'>
				<div class="col-lg-1">
					<div class="height4"></div>
	...
	<?php if ( $show_field ) : ?>
		<div class="col-lg-3">
			<div class='filter-unit-wrap woobe_post_author_edit <?php echo esc_attr( $fields[ $current_field_key ]['css_classes'] ); ?>
			<?php
			if ( ! $WOOBE->settings->get_fields()[ $current_field_key ]['direct'] ) :
				?>
				woobe-direct-field<?php endif; ?>'>
				...
				<div class="col-lg-3">
					...
					WOOBE_HELPER::draw_select_e(
						array(
							'disabled'   => 1,
							'options'    => array(
								'1' => esc_html__( 'Visible', 'woo-bulk-editor' ),
								'0' => esc_html__( 'Hidden', 'woo-bulk-editor' ),
							),
