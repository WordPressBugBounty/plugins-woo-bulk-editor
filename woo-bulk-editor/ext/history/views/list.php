            ...
            <li class="woobe_options_li <?php echo($is_solo ? 'solo_li' : 'bulk_li') ?> woobe_history_item woobe_history_li_show" id="woobe_history_<?php echo esc_attr(($is_solo ? $operation['id'] : $operation['bulk_key'])) ?>">
                ...
                <div class="col-lg-4">
                    ...
                    <?php else: ?>
                        ...
                        [<span style="color: <?php echo esc_attr($operation['state'] == 'completed' ? 'green' : 'red') ?>;"><small><?php printf(esc_html__('state: %s', 'woo-bulk-editor'), esc_html($operation['state'])) ?></small></span>]
                    <?php endif; ?>
                </div>
                ...
                <div class="col-lg-3">
                    <small>
                        <?php
                        if ($is_solo) {
                            echo esc_html(date(get_option('date_format') . ' ' . get_option('time_format'), $operation['mod_date']));
                        } else {
                            echo esc_html(date(get_option('date_format') . ' ' . get_option('time_format'), $operation['started']));
                            if ($operation['state'] !== 'terminated') {
                                echo esc_html(' - ' . date(get_option('date_format') . ' ' . get_option('time_format'), $operation['finished']));
                            }
                        }
                        ?>
                    </small>
